...
class JsonItem(Row):
    ...
    def cell(self, contents: str, type:str, **kwargs):
        self.file.write(f"<{json_escape(type)}>{json_escape(contents)}</{json_escape(type)}>\n")
...
class JsonList(Table):
    ...
    def __enter__(self) -> JsonList:
        ...
        if "title" in self.kwargs:
            self.file.write(
                f"<table title='{json_escape(self.kwargs['title'])}' >\n"
            )
        else:
            self.file.write(f"<table>\n")
...
class JsonSection(Section):
    ...
    def __enter__(self) -> JsonSection:
        # if "title" in self.kwargs:
        #     self.file.write(f"<h1>{json_escape(self.kwargs['title'])}</h1>\n")
        self.file.write(f'"section" : {{\n')
        self.file.write(f'\t"name" : "{attr_class(self.kwargs)}",\n')
        self.file.write(f"\t'tables' : [ \n")
        return self
